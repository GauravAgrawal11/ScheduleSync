...
## 🎯 8. Demo Walkthrough & Hackathon Judge Credentials

### Demo Credentials & 6 Supervisor Personas
The system is pre-seeded with **6 Field Supervisors** partitioned across 3 specialized engineering disciplines (2 Piping, 2 Civil, 2 Electrical) at Numaligarh Refinery, plus the **Central Lead Planner**:

| Discipline / Role | Supervisor Persona | Login Email / Username | Password | Assigned Scope / Location |
|---|---|---|---|---|
| **Piping** | **Supervisor 1 - Piping** (Biren Das) | `piping.sup1@oilindia.in` | `SecureSupervisorPassword123!` | Line 24 & 31 Erection, Spools, Valves (Unit 3) |
| **Piping** | **Supervisor 2 - Piping** (Dipak Kalita) | `piping.sup2@oilindia.in` | `SecureSupervisorPassword123!` | Manifolds, Welding NDT, Flanges (Unit 3) |
| **Civil** | **Supervisor 1 - Civil** (Sanjay Supervisor) | `supervisor@oilindia.in` | `SecureSupervisorPassword123!` | Footings, Foundations & Trenches (Unit 3) |
| **Civil** | **Supervisor 2 - Civil** (Manoj Bora) | `civil.sup2@oilindia.in` | `SecureSupervisorPassword123!` | Boundary Walls, Concrete Slabs & Roads |
| **Electrical** | **Supervisor 1 - Electrical** (Rajesh Das) | `electrical.sup1@oilindia.in` | `SecureSupervisorPassword123!` | Substation Cable Trays & Transformer Bays |
| **Electrical** | **Supervisor 2 - Electrical** (Kiran Saikia) | `electrical.sup2@oilindia.in` | `SecureSupervisorPassword123!` | Switchgear Panels, Conduit Pulls & Earthing |
| **Central Planning** | **Lead Project Planner** (Admin) | `planner@oilindia.in` | `SecurePlannerPassword123!` | Master Schedule, Review Queue & Approvals |

> 💡 **1-Tap Quick Select**: On the login screens (`/supervisor/login` and `/admin/login`), you do not need to type passwords manually—simply click any supervisor persona card to pre-fill their credentials and sign in instantly!

---

### Step-by-Step Hackathon Judge Demo Script

#### Test 1: Zero-Connectivity Field Offline Queue (PWA)
1. Navigate to `http://localhost:5173/supervisor/log`.
2. Open Browser DevTools (`F12`) $\to$ **Network** tab $\to$ set throttling to **Offline**.
3. Notice the top header immediately displays the amber **"Offline Mode"** pill.
4. Dictate or select a quick scenario (e.g. *Erect Line 24*), tap **"Submit Report for AI Extraction"**.
5. The submission succeeds locally with the explicit confirmation:
   > *"Saved — will send automatically when you're back online."*
6. Navigate to `http://localhost:5173/supervisor/submissions`:
   - The queued report appears at the top with an amber badge: `"Queued — will send when online"`.
7. Toggle DevTools Network back to **Online**:
   - Within 3 seconds, the background sync manager flushes the queue.
   - The queued item automatically disappears and appears in real server history with AI match scores!

#### Test 2: 3-Signal Match & Planner Verification Loop
1. As a supervisor, submit: *"Completed bolt-up flange on Line 24 in Unit 4"*.
2. Log into the Planner Cockpit at `http://localhost:5173/planner`.
3. Open the **Review Queue**:
   - View candidate match `L6-PIP-110` with exact 3-signal breakdown (Semantic 88%, Entity 100%, WBS 100%).
4. Click **"Approve Match"**:
   - Progress write-back updates the Gantt timeline in real time.
5. In Supervisor Submissions, the report now proudly displays: **"🛡️ Planner Verified"**.

#### Test 3: Out-of-Sequence Predecessor Violation
1. In the Planner Dashboard, notice the **Out-of-Sequence Alerts** card.
2. Observe active violation on `L6-PIP-111` (*Pressure gauge install*) because predecessor `L6-PIP-110` (*Bolt-up flange*) was incomplete at the time of execution.
3. Click **"Acknowledge"** to log planner review in the immutable audit trail.

#### Test 4: Automated HSE Safety Auto-Tagging
1. Submit a report containing: *"Hydrotesting completed. Near miss observed when scaffolding clamp slipped. No injury occurred."*
2. Open the Planner Cockpit $\to$ **Complaints & HSE Panel**.
3. Notice a new ticket auto-created under **Safety Concern**:
   > `[Auto-tagged HSE] Flagged keyword 'near miss': Hydrotesting completed. Near miss observed...`
4. The report ingestion API completed in 200ms with zero disruption to the supervisor.
